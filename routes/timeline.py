...
@router.get("/timeline/", response_model=List[TimelineOut], summary="List all timeline events")
def get_timeline_events():
    return [serialize_doc(evt) for evt in timeline_collection.find({})]

@router.put("/timeline/{event_id}", response_model=TimelineOut, dependencies=[Depends(get_current_user)], summary="Update a timeline event by id")
...
@router.delete("/timeline/{event_id}", dependencies=[Depends(get_current_user)], summary="Delete a timeline event by id")
